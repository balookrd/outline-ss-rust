    ...
    fn ensure_session_key(&mut self) -> Result<(), CryptoError> {
        ...
        for user in self.users.iter() {
            if user.cipher.is_2022() {
                ...
                let mut encrypted_fixed =
                    self.buffer[salt_len..salt_len + fixed_len].to_vec();
...
pub fn encrypt_udp_packet_for_response(
    ...
    match session {
        ...
        UdpSession::Aes2022 { client_session_id } => {
            ...
            let session_key =
                derive_subkey(user.cipher, &user.master_key, &server_session_id)?;
            let algorithm = cipher_algorithm(user.cipher);
            let key = UnboundKey::new(algorithm, &session_key).map_err(|_| CryptoError::Cipher)?;
            let less_safe = LessSafeKey::new(key);
            less_safe
        ...
        .map(|user| {
            if user.cipher.is_2022() {
                ...
                let key = match UnboundKey::new(algorithm, &session_key) {
                    Ok(key) => key,
                    Err(_) => return format!("{}:{} key_init_failed", user.id(), user.cipher.as_str()),
                };
                let less_safe = LessSafeKey::new(key);
        ...
        .map(|user| {
            if user.cipher == CipherKind::Chacha20Poly13052022 {
                ...
                let separate_header = match decrypt_ss2022_separate_header(user, encrypted_header) {
                    Ok(header) => header,
                    Err(_) => return format!("{}:{} separate_header_auth_failed", user.id(), user.cipher.as_str()),
                };
                let session_key =
...
fn parse_ss2022_request_header(header: &[u8]) -> Result<Vec<u8>, CryptoError> {
    let Some((target, consumed)) = parse_target_addr(header).map_err(|_| CryptoError::InvalidHeader)? else {
        return Err(CryptoError::InvalidHeader);
    };
    ...
    let padding_len =
        u16::from_be_bytes([header[consumed], header[consumed + 1]]) as usize;
...
fn parse_ss2022_udp_request_body(body: &[u8]) -> Result<Vec<u8>, CryptoError> {
    ...
    let timestamp = u64::from_be_bytes(body[1..9].try_into().map_err(|_| CryptoError::InvalidHeader)?);
    ...
    let Some((target, consumed)) = parse_target_addr(body).map_err(|_| CryptoError::InvalidHeader)? else {
        return Err(CryptoError::InvalidHeader);
    };
    let mut output = target.encode().map_err(|_| CryptoError::InvalidHeader)?;
...
fn parse_ss2022_chacha_udp_request_body(
    body: &[u8],
...
fn validate_ss2022_request_fixed_header(header: &[u8]) -> Result<usize, CryptoError> {
    ...
    let timestamp = u64::from_be_bytes(header[1..9].try_into().map_err(|_| CryptoError::InvalidHeader)?);
    validate_timestamp(timestamp)?;
...
fn derive_subkey(
    ...
    if cipher.is_2022() {
        ...
        Ok(blake3::derive_key(SS2022_SUBKEY_CONTEXT, &material).as_slice()[..cipher.key_len()].to_vec())
    } else {
...
fn encrypt_ss2022_separate_header(
    ...
    match user.cipher {
        CipherKind::Aes128Gcm2022 => {
            let cipher = Aes128::new_from_slice(&user.master_key).map_err(|_| CryptoError::Cipher)?;
            cipher.encrypt_block(&mut block);
        }
        CipherKind::Aes256Gcm2022 => {
            let cipher = Aes256::new_from_slice(&user.master_key).map_err(|_| CryptoError::Cipher)?;
            cipher.encrypt_block(&mut block);
        }
...
mod tests {
    ...
    use super::{
        ...
        encrypt_udp_packet, encrypt_udp_packet_for_response,
    };
    ...
    fn roundtrip_ss2022_tcp_stream() {
        ...
        let session_key = super::derive_subkey(CipherKind::Aes128Gcm2022, &users[0].master_key, &request_salt).unwrap();
        let key = LessSafeKey::new(UnboundKey::new(super::cipher_algorithm(CipherKind::Aes128Gcm2022), &session_key).unwrap());
        let mut nonce_counter = 0;

        let mut fixed_header = Vec::from([
            super::SS2022_TCP_REQUEST_TYPE,
        ]);
        ...
        key.seal_in_place_append_tag(super::next_stream_nonce(&mut nonce_counter), Aad::empty(), &mut fixed_ct).unwrap();
        ...
        key.seal_in_place_append_tag(super::next_stream_nonce(&mut nonce_counter), Aad::empty(), &mut var_ct).unwrap();
