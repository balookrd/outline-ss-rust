...
fn rejects_empty_session_id() {
    let xhttp_paths = paths(&["/xh"]);
    // `<base>/` (empty id, no seq).
    assert!(match_xhttp_path("/xh/", &xhttp_paths).is_none());
    // `<base>//<seq>` (empty id, has seq) — the `split_once` would
    // hand back an empty `id`, which we reject.
...
fn rejects_path_outside_base() {
    ...
    // Path equal to base — no id at all.
    assert!(match_xhttp_path("/xh", &xhttp_paths).is_none());
}
