...
2. Если SNI совпал с любой записью в `match_sni` (case-insensitive; `*.foo.bar` матчит ровно один лейбл слева), захваченные байты возвращаются в наш TLS-терминатор через обёртку `PrependStream`. Multi-cert резолвер из §7 выбирает сертификат; всё ниже по стеку — `[http_fallback]`, websocket / xhttp маршруты и т.п. — работает ровно как на не-сплайснутом коннекте.
3. Иначе (или если SNI не было и `allow_no_sni = false`) — выбираем бэкенд. В single-backend режиме это всегда `backend`. В multi-backend режиме идём по `backends` сверху вниз, выбираем первый, чей `match_sni` совпал (или catch-all); если ни один не подошёл — закрываем коннект с `WARN`-логом.
4. Открываем новое TCP-соединение к выбранному бэкенду, опционально префиксим HAProxy PROXY-protocol header, пишем захваченный ClientHello и `tokio::io::copy_bidirectional` пока кто-нибудь не закроет.
