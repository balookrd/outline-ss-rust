...
| `h3_cert_path` / `h3_key_path` | Обязательны для включения QUIC-эндпоинта |
...
Требует встроенного TLS (`tls_cert_path` + `tls_key_path`) на основном TCP-листенере.
...
match_sni = ["our.example.com",
             "*.api.our.example.com"]    # обязательно, nginx-style wildcards
...
2. Если SNI совпал с любой записью в `match_sni` (case-insensitive; `*.foo.bar` матчит ровно один лейбл слева), захваченные байты возвращаются в наш TLS-терминатор через обёртку `PrependStream`, и дальше всё работает как раньше — включая `[http_fallback]` если он включён.
3. Иначе (или если SNI не было и `allow_no_sni = false`) — открываем новое TCP-соединение к `backend`, опционально префиксим HAProxy PROXY-protocol header, пишем туда захваченный ClientHello и `tokio::io::copy_bidirectional` пока кто-нибудь не закроет.
...
- Wildcard'ы — только один лейбл слева (nginx-style). Без mid-segment wildcards и без полноценных регексов.
