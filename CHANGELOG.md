...
- Added cross-transport session resumption for SS-over-WebSocket and single-target VLESS-over-WebSocket (opt-in via `[session_resumption]`, off by default). When enabled, the server mints a 16-byte Session ID, returns it in the `X-Outline-Session` response header on WebSocket Upgrade (HTTP/1.1, HTTP/2, HTTP/3), and parks the live upstream `TcpStream` into an in-memory orphan registry on disconnect instead of tearing it down. A subsequent connect on any WebSocket transport carrying `X-Outline-Resume: <hex>` re-attaches to the parked upstream after authenticating the same user, skipping the upstream connect. The parked entry records which proxy protocol authenticated the original session — a resume request through a different protocol (SS↔VLESS) is rejected. VLESS mux sessions are not parked in this stage (see "Non-Goals" in the spec). Owner-mismatched resumes are reported externally as `unknown` to avoid an existence oracle. Per-user (`orphan_per_user_cap = 4`) and global (`orphan_global_cap = 10000`) caps bound memory; a periodic sweeper evicts entries past `orphan_ttl_tcp_secs` (default 30 s). New metrics: `outline_ss_orphan_park_total`, `_resume_hit_total`, `_resume_miss_total{reason}`, `_evicted_total{kind,reason}`, `_current{kind}`. UDP, VLESS-mux and raw-QUIC paths remain out of scope for this stage; see `docs/SESSION-RESUMPTION.md` for the wire format and roadmap.
