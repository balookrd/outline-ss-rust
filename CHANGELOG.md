...
- Add an `xhttp_h1` variant to the `Protocol` metric label so XHTTP packet-up traffic that lands on HTTP/1.1 stays inside the XHTTP family on the `protocol` label instead of falling through to plain `http1`. Required because the URI change above makes h1 reachable for packet-up — `protocol_from_http_version` previously mapped h1 to `Protocol::Http1` as an "unreachable fallback to keep this function total", and shipping h1 packet-up without updating it would have collapsed h1 XHTTP into the WebSocket-Upgrade bucket on the metrics dashboard, hiding the very split the `XhttpH2` / `XhttpH3` variants were introduced to make visible. Stream-one still returns 505 on h1 upstream and never reaches the metric path. `Protocol::VARIANTS_COUNT` bumped 7 → 8; the per-user `CounterMatrix` widens by one column. Existing scrapes keep working — Prometheus tolerates new label values.
