...
- Добавлено end-to-end покрытие XHTTP между репозиториями: новый тест-модуль поднимает настоящий клиент из `outline-ws-rust` (соседний репо, подключён как dev-dep по относительному пути) и гоняет его против этого сервера в одном tokio-процессе. Три кейса на plain-TCP h2 — packet-up round-trip, stream-one round-trip и resume между двумя последовательными dial-ами с одним и тем же токеном `X-Outline-Session` (тест гонит uplink-EOF напрямую через `XhttpSession::close_uplink`, поскольку у клиента пока нет FIN-сигнала). Кейсы h3 и TLS отложены до появления у клиента ручки для custom-cert TLS-конфига.
