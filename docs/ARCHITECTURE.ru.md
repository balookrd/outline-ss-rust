...
4. Иначе (или если SNI не было и `allow_no_sni = false`), диспатчер выбирает бэкенд и сплайсит соединение. Поддерживаются два режима конфигурации:
   - **Один бэкенд** (`backend = "host:port"` на уровне секции): все чужие SNI идут к одному upstream.
   - **Несколько бэкендов** (массив `[[sni_fallback.backends]]`): каждая запись содержит `backend`, опциональный `match_sni` (список SNI, роутимых именно сюда) и опциональный `proxy_protocol`. Диспатчер перебирает список по порядку и выбирает первую запись, чей `match_sni` содержит SNI соединения. Запись без `match_sni` (или с пустым списком) — catch-all и должна стоять последней; соединение, не подошедшее ни к одному бэкенду, молча сбрасывается. Это позволяет роутить разные домены к разным upstream (например, nginx на `:8443` для одного домена, caddy на `:9443` для другого) без отдельного haproxy впереди.
